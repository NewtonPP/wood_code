# woodchip_core/geometry.py
"""
Math / preprocessing / postprocessing helpers (pure numpy + cv2, no TRT/CUDA).

These are the canonical implementations; ``live_cam_trt.geometry`` re-exports
them so the Jetson device and the cloud share one code path.
"""
...
def compute_LWD(boxes_xyxy, pixels_per_mm=None):
    """Return L,W,D. In mm when ``pixels_per_mm`` is provided, else in pixels.

    Note: unlike the original (which read a module global), the scale is passed
    explicitly so each camera stream can carry its own calibration.
    """
    if boxes_xyxy.shape[0] == 0:
